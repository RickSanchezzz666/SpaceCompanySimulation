...
public:
	NeptunePlanet(std::string name, long double mass, float radius, float day, long double area,
		...
		int magneticField, float lifeChance, int moonsNum, int time, std::vector<std::string> moons, int id, bool belt, AsteroidCluster* cluster) : PlanetAbstract(name, mass, radius, day,
			...
			gravity, magneticField, lifeChance, moonsNum, time, moons, id, belt, cluster) { std::cout << "Neptune Planet Generated...\n"; };
	...
	NeptunePlanet() : PlanetAbstract("Neptune", 1.02413E26L, 24622.0f, 16.11f, 7.618E9L, 60190.0f, 30.05f, 72, 46, true, {
		...
		}, 1.14f, 7200, 0.001f, 14, 800, { "Triton", "Nereid", "Naiad", "Thalassa", "Despina", "Galatea", "Larissa", "Proteus",
